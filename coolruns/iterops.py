...
from coolruns.patterns import Singleton
...
class CallableDeque(deque):
    ...
    def __call__(self, result: np.ndarray, columns: Iterable[str]):...
...
class IterOps(metaclass=Singleton):
    def __init__(self, maxw: int = round(os.cpu_count()*0.75)):
        self.maxw: int = maxw
    def __call__(self, pack: CallableDeque, oper: Callable, columns: Iterable[str], **kwargs) -> Any:
