...
from coolruns._options import opt as options

#pd.options.mode.copy_on_rite = True
#pd.options.mode.string_storage = "pyarrow"
#pd.options.future.infer_string = True

__all__ = ["CoolRunnings", "options"]
...
class CoolRunnings(metaclass=AcessorPersistor):
    ...
    def __init__(self, df: pd.DataFrame):
        ...
        self.root = RootDimm(data=df) #TODO:: Invalid state Controll!!
    ...
    @overload
    def __call__(self, length: int, name: str = None, *args, **kwargs) -> Self: ...
    @overload
    ...
    def __call__(self, *args, **kwargs) -> Self:
        ...
        if args:
            if isinstance(args[0], int):
                return self.__single_roll(dict(list(zip(("length", "name", "stride"), args))+list(kwargs.items())))
...
if __name__ == "__main__":
    ...
    data.coolruns(8, "dimm1")
    data.coolruns(16, "dimm2")
    data.coolruns(8, "dimm3")
