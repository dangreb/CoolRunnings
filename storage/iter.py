...
class IterOps:
    ...
    def __init__(self, maxsize=None, timeout: float = None):
        maxsize = maxsize or opt.default_max_queue_size
        self.queue = MasterQueue(maxsize=maxsize, timeout=timeout)
    ...
    class IterOpsBroker(threading.Thread):
        ...
        def __call__(self, *args, **kwargs):
            while True:
                ...
                time.sleep(opt.threading_queue_loop_lag)


    def __init__(self, *args, **kwargs):
        super(QueueBroker, self).__init__(target=self, name="CoolRunnings.QueueBroker", daemon=False, *args, **kwargs)
        self.stop = threading.Event()
        self.iobk: QueueBroker.IterOpsBroker = QueueBroker.IterOpsBroker(self.stop)
...
class MasterQueue(queue.Queue, metaclass=Singleton):
    #def __init__(self, maxsize: int = opt.default_max_queue_size, timeout: float = None):
    def __init__(self, *args, **kwargs):
        super(MasterQueue, self).__init__(maxsize=maxsize)
        self.timeout = timeout
    def __call__(self, maxsize: int = None, timeout: float = None):
        self._init(maxsize)
        self.timeout = timeout
        return self
    def get(self, block=True, timeout=None):
        super(MasterQueue, self).get(block, self.timeout)
